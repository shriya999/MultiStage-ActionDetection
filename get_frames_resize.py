...
def get_args():
    ...
    parser.add_argument(
        "--statspath",
        default=None,
        help="path to write videoname.p to save some stats for " "that video",
    )
    parser.add_argument(
        "--use_2level", action="store_true", help="make videoname/frames dir"
    )
    parser.add_argument(
        "--name_level",
        type=int,
...
if __name__ == "__main__":
    ...
    for line in tqdm(open(args.videolist, "r").readlines()):
        ...
        vcap = VideoReader(video, "./meva_outdoor_test")
        frame_count = int(vcap.length)

        if cv2.__version__.split(".") != "2":
            frame_width = vcap.width
            frame_height = vcap.height
        ...
        while cur_frame < frame_count:
            ...
            cv2.imwrite(
                os.path.join(targetpath, "%s_F_%08d.jpg" % (videoname, cur_frame)),
                frame,
            )
